...
//!
//! Each body is wrapped in an explicit `unsafe` block rather than leaning on
//! the function's own `unsafe` marker, so `unsafe_op_in_unsafe_fn` stays
//! enabled here -- it is the lint that flags an unaudited raw-pointer op, and
//! the scalar tail loops (`*pa.add(i)`) are exactly what the per-function
//! safety notes below are about.
